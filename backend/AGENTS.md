...
This is a FastAPI backend that serves the static NextJS export at `/` and provides
API endpoints for board state, AI chat, and health checks. SQLite persists the
board state. Dummy auth is handled entirely on the frontend.

`.env` is loaded from the project root at app startup and in tests. OpenRouter is
used for AI calls, with `OPENROUTER_API_KEY` required and `OPENROUTER_MODEL`
